...
function MobileHeaderMenu({show,setShow}) {
  return (
    <div onClick={() => setShow(false)} className={`sm:hidden ${show ? "fixed" : "hidden"} top-0 left-0 w-full h-full bg-black text-white z-10`}>
        ...
        <div className='flex justify-center'>
            ...
            <img className='w-[40vw] rounded-full' src="img/logo.jpg" alt="" />
        </div>
      <ul  className='text-xl p-10 items-center flex flex-col justify-center'>
        <NavLink  className={""} to = "/"><li>Home</li></NavLink>
        <NavLink  className={"mt-10"} to = "/product"><li>Product</li></NavLink>
        <NavLink className={"mt-10"} to = "/about"><li>About</li></NavLink>
        <NavLink className={"mt-10"} to = "/contact"><li>Contact</li></NavLink>
        <NavLink  className={"mt-10"} to = "/custom"><li>Custom Order</li></NavLink>
      </ul>
    </div>
  )
}
